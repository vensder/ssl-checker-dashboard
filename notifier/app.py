...
if "NOTIFY_IF_DAYS_LEFT" in environ and environ["NOTIFY_IF_DAYS_LEFT"]:
    notify_if_days_left = environ["NOTIFY_IF_DAYS_LEFT"]

notify_every_n_hours = 0.5
if "NOTIFY_EVERY_N_HOURS" in environ and environ["NOTIFY_EVERY_N_HOURS"]:
    notify_every_n_hours = environ["NOTIFY_EVERY_N_HOURS"]
            ...
            if r.hget(host, "exp"):
                ...
                if expire_unix_time.isnumeric() or type(expire_unix_time) is int:
                    if (
                        ...
                        if not r.hget(host, "notified") or not strtobool(r.hget(host, "notified").decode("utf-8")):
...
if __name__ == "__main__":
    ...
    schedule.every(5).seconds.do(notify_expiring_soon)
